...
struct CustomSplitInfo_SlidingSingleMultiDeltaPostProcess
{
...
static vector<CustomSplitInfo_SlidingSingleMultiDeltaPostProcess> recfilter_split_info;
...
static RecFilterFunc create_final_tile_term(
	RecFilterFunc rF,
	vector<CustomSplitInfo_SlidingSingleMultiDeltaPostProcess> split_info)
{
	...
	{
		CustomSplitInfo_SlidingSingleMultiDeltaPostProcess s = split_info[scan[i].first];
...
// SingleSlidingと違うのはここだけ
/// <summary>
///　各deltaに各フィルターをかけたfuncを生成する
/// </summary>
/// <param name="rF"></param>
	...
	vector<CustomSplitInfo_SlidingSingleMultiDeltaPostProcess> split_info)
{
	...
	CustomSplitInfo_SlidingSingleMultiDeltaPostProcess s = split_info[0];
	...
	for (int j = 0; j < order; j++)
	{
		...
		int feedfwd_order = s.feedfwd_coeff_expr[curr_scan].size();
		int feedback_order = s.feedback_coeff_expr[curr_scan].size();

		// 対応するdeltaを元にフィルターをかける
		Function F = delta[curr_scan].function();
		...
		// deltaもsplitする
		{
			vector<string> pure_args = rF.func.args();
			vector<Expr>   pure_values = F.values();
		...
		// skinのRecFilterではなく、k番目のdeltaを参照するようにする
		// argは同じなのでskinので良い
		vector<string> pure_args = rF.func.args();
		vector<Expr>   pure_values = F.values();

		F_cac[curr_scan] = Function(F.name() + DASH + std::to_string(j));
		F_cac[curr_scan].define(pure_args, undef_values);
	...
	vector<CustomSplitInfo_SlidingSingleMultiDeltaPostProcess> split_info)
{
	...
	CustomSplitInfo_SlidingSingleMultiDeltaPostProcess sInfo = split_info[0];
...
/// <param name="split_info"></param>
static void merge_init_and_filtering_term(vector<RecFilterFunc>& rF_final_calc, vector<CustomSplitInfo_SlidingSingleMultiDeltaPostProcess>& split_info)
{
	if (split_info.size() >= 2)
	{
	...
	{
		int feedfwdOrder = sInfo.feedfwd_coeff_expr[k].size();
		int feedbackOrder = sInfo.feedback_coeff_expr[k].size();
		const int order = std::max(feedfwdOrder, feedbackOrder);
		const int fiter_start_index = std::max(std::max(feedfwdOrder - 1, feedbackOrder), 0);
...
void RecFilter::split_custom_sliding_single_multi_delta_post_process(map<string, int> dim_tile)
{
	...
	for (map<string, int>::iterator it = dim_tile.begin(); it != dim_tile.end(); it++)
	{
		...
		for (int j = 0; !found && j < contents.get()->filter_info.size(); j++)
		{
			...
			CustomSplitInfo_SlidingSingleMultiDeltaPostProcess s;
	...
	vector<RecFilterFunc> rF_final_cac;
	{
		...
		if (delta.size() != num_scan)
		{
			std::cerr << "The number of filters and delta functions do not match\n";
			assert(false);
		}

		rF_final = create_final_tile_term(rF, recfilter_split_info);
		rF_final_cac = create_filter_funcs(rF_final, delta, recfilter_split_info);
