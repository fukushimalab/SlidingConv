...
struct CustomSplitInfo_SlidingSingleMultiDelta
{
...
static vector<CustomSplitInfo_SlidingSingleMultiDelta> recfilter_split_info;
...
static RecFilterFunc create_final_tile_term(
	RecFilterFunc rF,
	vector<CustomSplitInfo_SlidingSingleMultiDelta> split_info)
{
	...
	{
		CustomSplitInfo_SlidingSingleMultiDelta s = split_info[scan[i].first];
	...
	vector<CustomSplitInfo_SlidingSingleMultiDelta> split_info)
{
	...
	CustomSplitInfo_SlidingSingleMultiDelta s = split_info[0];
	...
	vector<CustomSplitInfo_SlidingSingleMultiDelta> split_info)
{
	...
	CustomSplitInfo_SlidingSingleMultiDelta sInfo = split_info[0];
...
/// <param name="split_info"></param>
static void merge_init_and_filtering_term(vector<RecFilterFunc>& rF_final_calc, vector<CustomSplitInfo_SlidingSingleMultiDelta>& split_info)
{
	if (split_info.size() >= 2)
	{
...
void RecFilter::split_custom_sliding_single_multi_delta(map<string, int> dim_tile)
{
	...
	for (map<string, int>::iterator it = dim_tile.begin(); it != dim_tile.end(); it++)
	{
		...
		for (int j = 0; !found && j < contents.get()->filter_info.size(); j++)
		{
			...
			CustomSplitInfo_SlidingSingleMultiDelta s;
