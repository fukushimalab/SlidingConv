...
private:
	...
	virtual RecFilterFunc& internal_function(std::string func_name);

	/** Get all recursive filter funcs that have the given tag */
	std::vector<std::string> internal_functions(FuncTag ftag);

	/** Get all recursive filter funcs that have the given type */
	std::vector<std::string> internal_functions(FuncType ftype);
	 ...
	 */
	virtual void inline_func(std::string func_name);
	...
	int Algorithm;
	int redundant;
	// switch parallelization. default:on
	bool parallel;

	bool is_inlined;

	bool use_custom_init;
	std::vector<Halide::Func> custom_init;
	std::vector<Halide::Func> delta;
	Halide::Func post_process;
	bool has_post_process = false;

	bool slidingScheduled = false;
...
public:
	...
	enum
	{
		...
		Custom = 0b10,
		SlidingSingle = 0b100,
		SlidingSinglePostProcess = 0b1000,
		SlidingSingleMultiDelta = 0b10000,
		SlidingSingleMultiDeltaPostProcess = 0b100000,
	};

	/** Empty constructor */
	RecFilter(std::string name, Halide::Target target);
	...
	void setTargetFeature(Halide::Target::Feature feature);
	 ...
	 */
	 // {@
	void add_filter(RecFilterDim x, std::vector<float> coeff, const int feedfwd_size);
	void add_filter(RecFilterDimAndCausality x, std::vector<float> coeff, const int feedfwd_size);


	// custom coeff
	void add_filter(RecFilterDimAndCausality x, std::vector<Halide::Expr> coeff, const int feedfwd_size);
	// @}
	...
	// ORIGINAL : original RecFilter
	// MIRROR : mirror edge
	// EXPAND : 
	void algorithm(int type)
	{
		assert(type >= Original && type <= SlidingSingleMultiDeltaPostProcess);

		//if (!use_custom_init && type & Custom)
		//{
		//	std::cerr << "Cannot specified algorithm to CustomRecFilter \n"
		//		<< "because RecFilter" << name() << "does not have custom initialization function,\n";
		//	assert(false);
		//}
	...
	virtual void split(RecFilterDim x, int tx);
	virtual void split(RecFilterDim x, int tx, RecFilterDim y, int ty);
	virtual void split(RecFilterDim x, int tx, RecFilterDim y, int ty, RecFilterDim z, int tz);
	virtual void split(std::map<std::string, int> dims);
	void split_custom_sliding_single(std::map<std::string, int> dims);
	void split_custom_sliding_single_post_process(std::map<std::string, int> dims);
	void split_custom_sliding_single_multi_delta(std::map<std::string, int> dims);
	void split_custom_sliding_single_multi_delta_post_process(std::map<std::string, int> dims);
	// @}
	...
	void store_at(RecFilter external);
	...
	void compute_with(RecFilter external);

	void store_at(Halide::Func external, Halide::Var granularity);
	void store_root();

	void trace_all_realizations();
	void store_in(Halide::MemoryType memory_type);
	...
	/// <summary>
	/// schedule for SingleSliding, SingleSlidingMultiDelta
	/// </summary>
	void cpu_auto_schedule_for_sliding(void);

	void cpu_auto_schedule_for_sliding_custom(void);

	void gpu_auto_schedule_for_sliding(void);

	void gpu_auto_schedule_for_sliding_custom(void);

	void schedule_for_sliding_custom(void);
	...
	/*void switch_order_of_storage(RecFilter consumer){
		switch_order_of_storage(consumer.as_func());
	}*/
	void switch_order_of_storage(RecFilter consumer);
	void switch_order_of_storage(std::vector<Halide::Func> funcs);

	void set_custom_init_Func(std::vector<Halide::Func> init_funcs)
	{
		this->custom_init = init_funcs;
		this->use_custom_init = true;
		this->Algorithm &= ~Original; // originalを消す
		this->Algorithm |= Custom;
	}

	void set_delta_Func(std::vector<Halide::Func> delta_funcs)
	{
		this->delta = delta_funcs;
	}

	void set_post_process_Func(Halide::Func post_func)
	{
		this->post_process = post_func;
		this->has_post_process = true;
	}

	void set_schedule(ScheduleInfo schedule)
	{
		this->schedule = schedule;
		this->slidingScheduled = true;

		schedule_for_sliding_custom();
	}

	VarTag mapScheduleTagToVarTag(ScheduleTag tag, bool isY);
...
public:
	...
	void operator=(const Halide::Tuple& pure_def); // 邪魔

	/**  Use this as the left-hand-side of a definition */
	void operator=(Halide::FuncRef pure_def);

	void operator=(RecFilterRefVar pure_def);
