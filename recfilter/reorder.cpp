...
{
	if (contents.get()->tiled || contents.get()->compiled || contents.get()->finalized)
	{
	...
	// check that the order does not violate
	{
		...
		{
			int scan_dim = contents.get()->filter_info[i].filter_dim;
			for (int j = 0; j < contents.get()->filter_info[i].num_scans; j++)
			{
				int scan_id = contents.get()->filter_info[i].scan_id[j];
				bool causal = contents.get()->filter_info[i].scan_causal[j];
	...
	for (int i = 0; i < contents.get()->filter_info.size(); i++)
	{
		args.push_back(RecFilterDim(
			contents.get()->filter_info[i].var.name(),
			contents.get()->filter_info[i].image_width));
	}
	...
	for (int i = 0; i < scans.size(); i++)
	{
		RecFilter rf(contents.get()->name + "_" + std::to_string(i));
		...
		if (i == 0)
		{
			rf(args) = as_func().values().as_vector();
		}
		else
		{
		...
		for (int j = 0; j < scans[i].size(); j++)
		{
			...
			int dim = -1;
			int idx = -1;
			for (int u = 0; dim < 0 && u < contents.get()->filter_info.size(); u++)
			{
				for (int v = 0; idx < 0 && v < contents.get()->filter_info[u].num_scans; v++)
				{
					if (scan_id == contents.get()->filter_info[u].scan_id[v])
					{
						dim = u;
						idx = v;
					}
			...
			RecFilterDim x(contents.get()->filter_info[dim].var.name(),
				contents.get()->filter_info[dim].image_width);
			bool causal = contents.get()->filter_info[dim].scan_causal[idx];
			//int order   = contents.get()->filter_info[dim].filter_order;
			int feedfwd_order = contents.get()->filter_info[dim].feedfwd_order;
			int feedback_order = contents.get()->filter_info[dim].feedback_order;

			vector<float> coeff;
			//int feedback_size = contents.get()->feedfwd_coeff.height();
			for (int u = 0; u < feedfwd_order; u++)
			{
				coeff.push_back(contents.get()->feedfwd_coeff(scan_id, u));
			}
			//coeff.push_back(contents.get()->feedfwd_coeff(scan_id));
			for (int u = 0; u < feedback_order; u++)
			{
				coeff.push_back(contents.get()->feedback_coeff(scan_id, u));
			}

			rf.add_filter((causal ? +x : -x), coeff, feedfwd_order);
		}
...
vector<RecFilter> RecFilter::cascade_by_causality(void)
{
	...
	for (int i = 0; i < contents.get()->filter_info.size(); i++)
	{
		for (int j = contents.get()->filter_info[i].num_scans - 1; j >= 0; j--)
		{
			int id = contents.get()->filter_info[i].scan_id[j];
			bool c = contents.get()->filter_info[i].scan_causal[j];
			if (c)
			{
...
vector<RecFilter> RecFilter::cascade_by_dimension(void)
{
	...
	for (int i = 0; i < contents.get()->filter_info.size(); i++)
	{
		...
		for (int j = contents.get()->filter_info[i].num_scans - 1; j >= 0; j--)
		{
			dim_scans.push_back(contents.get()->filter_info[i].scan_id[j]);
		}
		if (!dim_scans.empty())
		{
...
RecFilter RecFilter::overlap_to_higher_order_filter(RecFilter fB, string overlap_name)
{
	...
	for (int i = 0; i < contents.get()->filter_info.size(); i++)
	{
		string var = contents.get()->filter_info[i].var.name();
		int width = contents.get()->filter_info[i].image_width;
		filter_dim.push_back(RecFilterDim(var, width));
	}

	// check that both filters have same border clamping
	bool border_clamp_a = contents.get()->clamped_border;
	...
	if (contents.get()->type != fB.contents.get()->type)
	{
	...
	Buffer<float> feedfwd_a = contents.get()->feedfwd_coeff;
	Buffer<float> feedfwd_b = fB.contents.get()->feedfwd_coeff;
	Buffer<float> feedback_a = contents.get()->feedback_coeff;
	Buffer<float> feedback_b = fB.contents.get()->feedback_coeff;

	// add the scans
	for (int i = 0; i < contents.get()->filter_info.size(); i++)
	{
		int  filter_dim_a = contents.get()->filter_info[i].filter_dim;
		int  num_scans_a = contents.get()->filter_info[i].num_scans;
		int  image_width_a = contents.get()->filter_info[i].image_width;
		int  tile_width_a = contents.get()->filter_info[i].tile_width;
		Var  var_a = contents.get()->filter_info[i].var;
		RDom rdom_a = contents.get()->filter_info[i].rdom;
		...
		{
			bool scan_causal_a = contents.get()->filter_info[i].scan_causal[j];
			int  scan_id_a = contents.get()->filter_info[i].scan_id[j];
			...
			int feedback_size = feedfwd_a.height();
			for (int u = 0; u < feedback_size; u++)
			{
				// feedforward coeff
				float ff = feedfwd_a(scan_id_a, u) * feedfwd_b(scan_id_b, u);
				// add the feedfowrd coeff to the list of coeff
				coeff.insert(coeff.begin() + u, ff);
			}

			if (scan_causal_a)
			{
				AB.add_filter(x, coeff, feedback_size);
			}
			else
			{
				AB.add_filter(-x, coeff, feedback_size);
