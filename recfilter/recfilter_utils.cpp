...
ostream& operator<<(ostream& os, const Internal::Function& f)
{
	...
	if (f.has_pure_definition())
	{
		...
		{
			if (f.updates()[j].defined())
			{
				bool already_added = false;
				for (int i=0; i<rdom.size(); i++)
				{
					already_added = f.updates()[j].same_as(rdom[i].);
				}
				if (!already_added)
				{
					rdom.push_back(f.updates()[j]);
				}
			}
		}
		if (!rdom.empty())
...
ostream& operator<<(std::ostream& os, const RecFilterFunc& f)
{
	...
	s << "// Function tag: " << f.func_category << endl;
	s << "// Function type: " << f.func_type << endl;
...
bool operator==(const FuncType& a, const FuncType& b) { return (a.as_integer() == b.as_integer()); }
bool operator!=(const VarTag& a, const VarTag& b) { return !(a == b); }
bool operator!=(const FuncTag& a, const FuncTag& b) { return !(a == b); }
bool operator!=(const FuncType& a, const FuncType& b) { return !(a == b); }
...
bool operator==(const FuncType& a, const FunctionType& b) { return (a == FuncType(b)); }

ostream& operator<<(ostream& s, const FunctionTag& f) { s << FuncTag(f); return s; }
ostream& operator<<(ostream& s, const FunctionType& f) { s << FuncType(f); return s; }
...
ostream& operator<<(ostream& s, const FuncType& f)
{
	if (f == NONE) { s << "NONE"; }
	if (f == SKIN) { s << "SKIN"; }
	if (f == FINAL) { s << "FINAL"; }
