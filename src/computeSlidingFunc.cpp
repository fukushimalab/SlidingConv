...
public:
	...
	SlidingConvFilterDCT(int w, int h, int d, int dctType, int order, int radius, std::function<double(int, double, int)> kernel, bool coeffOptimize = true) : w(w), h(h), d(d), dctType(dctType), order(order), radius(radius)
	{
...
vector<Func> SlidingConv::computeSlidingFunc(Buffer<float> inputX, bool asSeparable)
{
	...
	case DCT1:
	{
		SlidingConvFilterDCT filter = SlidingConvFilterDCT(content.width, content.height, content.channels, 1, content.order, content.radius, content.kernel, content.optimizeCoeff);
	...
	case DCT3:
	{
		SlidingConvFilterDCT filter = SlidingConvFilterDCT(content.width, content.height, content.channels, 3, content.order, content.radius, content.kernel, content.optimizeCoeff);
		vector<Func> filters = filter.filter(Func(content.internalFunction), inputX, content.scheduleX.splitX, content.scheduleY.splitY, content.has_post_process, content.post_process, content.scheduleX, content.scheduleY, asSeparable, content.target);
		return filters;
	}
	...
	case DCT5:
	{
		SlidingConvFilterDCT filter = SlidingConvFilterDCT(content.width, content.height, content.channels, 5, content.order, content.radius, content.kernel, content.optimizeCoeff);
		vector<Func> filters = filter.filter(Func(content.internalFunction), inputX, content.scheduleX.splitX, content.scheduleY.splitY, content.has_post_process, content.post_process, content.scheduleX, content.scheduleY, asSeparable, content.target);
		return filters;
	}
	...
	case DCT7:
	{
		SlidingConvFilterDCT filter = SlidingConvFilterDCT(content.width, content.height, content.channels, 7, content.order, content.radius, content.kernel, content.optimizeCoeff);
		vector<Func> filters = filter.filter(Func(content.internalFunction), inputX, content.scheduleX.splitX, content.scheduleY.splitY, content.has_post_process, content.post_process, content.scheduleX, content.scheduleY, asSeparable, content.target);
		return filters;
	}
	}
